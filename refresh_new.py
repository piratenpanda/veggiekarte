...
f = urllib2.urlopen('http://overpass-api.de/api/interpreter?data=[out:json];(node["diet:vegan"~"yes|only"];node["diet:vegetarian"~"yes|only"];way["diet:vegan"~"yes|only"];way["diet:vegetarian"~"yes|only"];>;);out;')
...
with open(scriptdir + '/veggiemap-data.js', 'w') as f:
  ...
  for e in json['elements']:
    ...
    if typ == 'node':
      nodes[ide] = (lat,lon)
      if tags.get('diet:vegan') != 'yes' and tags.get('diet:vegan') != 'only' and tags.get('diet:vegetarian') != 'yes' and tags.get('diet:vegetarian') != 'only':
        continue
